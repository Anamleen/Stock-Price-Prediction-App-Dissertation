...
def add_indicators(df):
    ...
    # Moving Averages (for main chart — same as SMA)
    df['MA_20']  = df['SMA_20']
    df['MA_50']  = df['SMA_50']
...
with st.spinner(f"Fetching {selected_stock} data..."):
    try:
        ...
        if len(full_data) < 200:
            st.error("Not enough historical data for this symbol.")
            st.stop()
...
c1.metric("Latest Close",   f"${latest:.2f}",
          f"{arrow} Daily Change: {change:+.2f} ({pct_chg:+.2f}%)", delta_color=color)
...
fig_main.add_trace(go.Bar(
    x=display_data['Date'], y=display_data['Volume'],
...
ohlcv['Daily Change'] = ohlcv['Daily Change'].apply(
    lambda x: f"▲ +${x:.2f}" if x >= 0 else f"▼ -${abs(x):.2f}" if not pd.isna(x) else "—")
ohlcv['Change %']     = ohlcv['Change %'].apply(
    lambda x: f"+{x:.2f}%" if x >= 0 else f"{x:.2f}%" if not pd.isna(x) else "—")
...
with tab_rsi:
    ...
    fig_rsi.update_layout(template="plotly_dark", height=440,
                          yaxis=dict(range=[0, 100]), title="RSI (14-period)",
                          margin=dict(l=10, r=10, t=50, b=10))
...
if predict_btn:
    with st.spinner("Running predictions..."):
        ...
        if len(disp_indices) == 0:
            ...
            st.stop()
        if disp_indices[0] < LOOKBACK:
            st.error("Not enough history before selected start date. Please pick a later start date.")
            st.stop()
